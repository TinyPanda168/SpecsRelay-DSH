...
仍然只需点击 **整理当前对话**，无需选择框架、安装其他工具或新增 Key。修订会返回完整现行需求；目前不提供稳定需求编号或持久化的语义版本对比。原有格式修复和可选长对话增强可能产生额外模型调用。模型输出仍需核对，验证范围与内容验收方式见[验收样例](requirement-analysis-evaluation.md)。更新插件后重启 DSH Desktop，以重新加载内置 Skill。
