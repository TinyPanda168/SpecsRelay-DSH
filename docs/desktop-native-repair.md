# DSH Desktop 原生网页修复与升级检查

SpecsRelay 的左右分栏需要桌面宿主提供 `desktopWebPanels`。2026-09-24 检查的官方 DSH Desktop 2.0.13 安装包没有该服务；此前可用的本地配套版本包含额外的原生网页实现。覆盖安装官方版本会移除这部分实现，重试或重新安装 SpecsRelay 插件不能恢复它。
...
后续官方更新仍可能覆盖配套实现。在上游正式包含该服务之前，每次升级都需要针对新版本重新适配并重复上述验收；仅看版本号不能证明兼容。本补丁不禁用官方更新，也不会自动改写新安装的应用。
