...
function optionsFromEnvironment(env) {
  return {
    ...
    omitProbability: numericOption(env, "SPECSRELAY_JEV_OMIT_PROBABILITY", LONG_CONVERSATION_DEFAULTS.omitProbability, 0.9, 1, false)
  };
}
...
async function filterEvidence(text, chunks, readingPlan, key, options, signal, fetchImpl) {
  ...
  for (let start = 0; start < candidates.length;) {
    ...
    let batch = candidates.slice(start, start + BATCH_SIZE);
    let body;
    while (batch.length > 0) {
      ...
      body = JSON.stringify({
        ...
        state: {
          ...
          fragments: [...context].sort((a, b) => a - b).map((index) => maskedChunks[index])
        },
        questions
      });
      if (Buffer.byteLength(body) <= MAX_REQUEST_BYTES) break;
      batch = batch.slice(0, -1);
    }
    if (batch.length === 0) throw new Error("Evidence batch exceeds the request budget.");
    const response = await fetchImpl(JEV_API_URL, {
      method: "POST",
 ...
 * @param {object} options Planner callback, parent signal, environment and fetch.
...
export async function enhanceLongConversation(ctx, text, {
  ...
  if (!/^(1|true|on|yes)$/i.test(String(env.SPECSRELAY_JEV_LONG_CONTEXT ?? "").trim())) return text;
  ...
  try {
    return await Promise.race([deadline, (async () => {
      ...
      requestSignal.throwIfAborted();
      return await filterEvidence(text, chunks, readingPlan, key, options, requestSignal, fetchImpl);
    })()]);
  } catch {
