...
**把 DeepSeek 里聊清楚的想法，交给 DSH 项目继续做。**

在 DSH Desktop 内打开 DeepSeek，点击一次整理完整对话。SpecsRelay 会提炼需求、请你确认关键问题，再把你确认的需求发送到目标项目并启动 Agent。

- **沿用熟悉的网页**：保留 DeepSeek 登录和历史会话。
- **复用已有模型**：使用 DSH 已配置的模型，无需重复填写模型 Key。
- **你来确认交接**：先核对需求和项目，再发送并开始处理。

> 仅支持 [anywhere-labs DSH Desktop](https://github.com/anywhere-labs/deepseek-harness-desktop)。本项目由社区独立维护，不是 DeepSeek 官方产品或桌面客户端的内置插件。

简体中文 | [English](README.en.md)

最近更新：**0.10.0（2026-09-21）** — [查看更新记录](CHANGELOG.md)
...
[快速安装](#快速安装) · [工作流程](#工作流程) · [可选功能](#可选功能) · [数据与安全](#数据与安全)

## 为什么做 SpecsRelay

聊清楚一个想法之后，往往还要复制长对话、整理已确认的决定，再告诉 Coding Agent 在哪个项目继续。SpecsRelay 帮你完成这段交接。

| 环节 | 负责什么 |
| --- | --- |
| DeepSeek | 讨论想法、比较方案、确认决定 |
| SpecsRelay | 整理目标、约束和验收标准，补齐待确认问题 |
| DSH Agent | 在你选定的本地项目中继续实现 |
...
安装识别、配置目录及窗口模式见 [DSH Desktop 集成说明](docs/desktop-client-adapters.md)。
...
- Jev 两项能力默认关闭；开启后的发送内容见 [Jev 数据说明](docs/jev-features.md#发送给-typesafe-的数据)。
