...
- **Laya HTTP adapter**: support `/v1/systemone` with per-candidate budgets, a separate threshold, full-source fallback and rejected redirects. Never switch to another service on failure. Mock HTTP, persistence and UI-switch tests cover integration, not live-model quality.
